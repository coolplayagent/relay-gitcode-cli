...
`GD_TOKEN` 和 `GITCODE_TOKEN` 可用于 CI 和临时端到端测试，其中
`GD_TOKEN` 优先级更高。没有环境变量 token 时，`gd auth login --with-token`
会把 token 保存到系统 keyring。
...
`NO_PROXY`/`no_proxy`。GitCode API 调用默认不校验 TLS 证书。设置
`GD_SSL_VERIFY`/`gd_ssl_verify`、`GITCODE_SSL_VERIFY`/`gitcode_ssl_verify`
或 `SSL_VERIFY`/`ssl_verify` 为 `true` 可启用证书校验；设置
`GIT_SSL_NO_VERIFY`/`git_ssl_no_verify` 为任意非空值可保持禁用校验。
...
可选仓库迁移端到端测试由环境变量驱动，只应用于一次性 GitCode 测试仓库：
