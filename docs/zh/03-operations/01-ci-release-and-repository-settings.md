...
`gd release migrate-github`。workflow 使用 `gd repo sync-github --method
git-push`，因此目标会保持为可写的普通 GitCode 仓库，而不是 Pull 镜像仓库。
该 token 需要具备创建或更新目标 GitCode 仓库、通过 HTTPS 推送分支和标签、
