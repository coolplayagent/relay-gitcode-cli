...
Low-level repository update endpoints that have broad GitCode-specific request
shapes accept repeated `--field key=value` entries. Values `true`, `false`,
`null`, and numbers are sent as JSON scalars; other values are sent as strings.
...
GitCode models pull request review as approval and test operations, so `gd pr
review` maps to GitCode approver/tester endpoints rather than GitHub review
states.
